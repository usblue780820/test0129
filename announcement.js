/**
 * 公休公告生成器 (字型選擇增強版)
 * 包含：智慧欄位讀取 + 圖片強制置頂 + 完整顯示模式 + 額外備註 + 新年預設圖 + 自訂字型選擇
 */
    ...
    if (fileInput) {
        ...
        if (line4Input && !document.getElementById('announce-extra')) {
            ...
            noteWrapper.innerHTML = `
                ...
                <textarea id="announce-extra" class="w-full border p-2 rounded text-sm focus:ring-blue-500" rows="3" placeholder="例如：歡迎提早預約！"></textarea>
            `;
...
function updateRenderOptions() {
    ...
    renderOptions.fontFamily = fontSelect ? fontSelect.value : 'default';
...
// 輔助函式：根據設定取得當前應使用的 Font Family
function getCurrentFont(baseStyle) {
    if (renderOptions.fontFamily === 'default' || !renderOptions.fontFamily) {
        return baseStyle.font;
    }
    // Handwritten5 對應 清風手寫體5, 851Font 對應 851手寫體
    return `${renderOptions.fontFamily}, Noto Sans TC, sans-serif`;
}
...
function drawAnnouncement() {
    try {
        ...
        // 取得動態字體
        const activeFont = getCurrentFont(style);
        ...
        if (customBgImage) {
            ...
            if (isContain) drawContainImage(ctx, customBgImage, canvas.width, canvas.height);
            else drawCoverImage(ctx, customBgImage, canvas.width, canvas.height);
        }
        ...
        ctx.font = `bold 34px ${activeFont}`;
        const w2 = ctx.measureText(subPart2).width;
        
        let currentTextX = centerX - ((w1 + w2 + w3) / 2); 
        ctx.textAlign = 'left';
        ctx.font = `bold 28px ${activeFont}`;
        ...
        roundRect(ctx, gridStartX, gridStartY, gridWidth, 50, 10, true); 
        ctx.fill();
        ...
        for (let i = 0; i < 6; i++) { 
            ...
            if (currentDay > daysInMonth) break;
            col = 0; currentX = gridStartX; currentY += cellHeight;
        }
        ...
        if (renderOptions.extraNote) {
            ...
            ctx.font = `bold 42px ${activeFont}`; 
            ctx.fillStyle = style.extraNoteColor; 
            ctx.textAlign = 'center';
            const lines = renderOptions.extraNote.split('\n');
            const lineHeight = 60;
            let textY = gapCenterY - ((lines.length * lineHeight) / 2) + (lineHeight / 3);
            lines.forEach(line => { ctx.fillText(line, centerX, textY); textY += lineHeight; });
        }

        // 5. 底部資訊
        ctx.fillStyle = style.footerBg;
        ctx.fillRect(0, footerStartY, canvas.width, 240);
        ctx.fillStyle = style.footerText;
        ctx.textAlign = 'left';
        ctx.font = `bold 32px ${activeFont}`;
        const textStartX = 100;
        let textY = footerStartY + 60;
        if(renderOptions.line1) { ctx.fillText(renderOptions.line1, textStartX, textY); textY += 50; }
        if(renderOptions.line2) { ctx.fillText(renderOptions.line2, textStartX, textY); textY += 50; }
        if(renderOptions.line3) { ctx.fillText(renderOptions.line3, textStartX, textY); textY += 50; }
        if(renderOptions.line4) { ctx.fillText(renderOptions.line4, textStartX, textY); }
    } catch (e) { console.error("繪製出錯:", e); }
}
...
function roundRect(ctx, x, y, width, height, radius, topOnly = false) {
    ...
    if(topOnly) { ctx.lineTo(x + width, y + height); ctx.lineTo(x, y + height); ctx.lineTo(x, y + radius); }
    else {
...
function drawCoverImage(ctx, img, w, h) {
    ...
    if (prop > ctxProp) { drawH = h; drawW = h * prop; startX = (w - drawW) / 2; startY = 0; }
    else { drawW = w; drawH = w / prop; startX = 0; startY = 0; }
